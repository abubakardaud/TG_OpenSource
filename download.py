...
class TemporalDataSets(object):
    ...
    def download_all(self):
        print(bcolors.WARNING + "attempting download all data, will remove ALL files and download ALL possible files" + bcolors.ENDC)
    ...
    def generate_split_masks(self, dataset_name, val_ratio=0.15, test_ratio=0.15, different_new_nodes_between_val_and_test=False,
        ...
        if save_indices:
            ...
            np.save('./{}/split_index/ml_{}_new_node_val_index.npy'.format(self.base_directory, dataset_name), train_mask)
            np.save('./{}/split_index/ml_{}_new_node_test_index.npy'.format(self.base_directory, dataset_name), train_mask)
    ...
    def get_data_link_pred_from_indices(self, dataset_name, split_masks=None, randomize_fatures=False):
        ...
        if (split_masks is not None):
            ...
            new_node_val_mask = np.load('./{}/split_index/ml_{}_new_node_val_index.npy'.format(self.base_directory, self.data_str))
            new_node_test_mask = np.load('./{}/split_index/ml_{}_new_node_test_index.npy'.format(self.base_directory, self.data_str))
    ...
    # '''
    # 1. get the data split, either based on a specified random seed or load a default split from the dataset 
    # 2.  
    ...
    # #         get_data_link_pred_from_indices(dataset_name, split_masks=No, randomize_fatures=False):
    # #     """
    # #     Load data based on the indices read from file
    # #     """
    # #     ### Load data and train val test split
    ...
    # #     edge_features = np.load('./data/ml_{}.npy'.format(dataset_name))
    # #     node_features = np.load('./data/ml_{}_node.npy'.format(dataset_name))
    # #                     ditional for CAW data specifically
    # #         ataset_name in ['enron', 'socialevolve', 'uci']:
    # #         node_zero_padding = np.zeros((node_features.shape[0], 172 - node_features.shape[1]))
    # #     node_features = np.concatenate([node_features, node_zero_padding], axis=1)
    # #      in notnot None:
    # #         train_mask = split_masks['train']
    ...
    # #     else:
    # #         # read index of the data splits
    # #         print("INFO: Read the split indices from the files...")dding = np.zeros((edge_features.shape[0], 172 - edge_features.shape[1]))
    # #             edge_features = np.concatenate([edge_features, edge_zero_padding], axis=1)
    # #                     # read index of the data splits
    # #         train_mask = np.load('./data/split_index/ml_{}_train_index.npy'.format(dataset_name))
    # #         val_mask = np.load('./data/split_index/ml_{}_val_index.npy'.format(dataset_name))
    # # test_mask = np.load('./data/split_index/ml_{}_test_index.npy'.format(dataset_name))
    # #     new_node_val_mask = np.load('./data/split_index/ml_{}_new_node_val_index.npy'.format(dataset_name))
    # #         node_test_mask = np.load('./data/split_index/ml_{}_new_node_test_index.npy'.format(dataset_name))
    ...
    # #     labels = graph_df.label.values
    # #     timestamps = graph_df.ts.values
    # #                      , destinations, timestamps, edge_idxs, labels)

    # #                     a(sources[train_mask], destinations[train_mask], timestamps[train_mask],
    # #                       edge_idxs[train_mask], labels[train_mask])
    # #                     ources[val_mask], destinations[val_mask], timestamps[val_mask],
    # #                     edge_idxs[val_mask], labels[val_mask])
    # #                             test_mask], destinations[test_mask], timestamps[test_mask],
    # #                             s[test_mask], labels[test_mask])
    # #     new_node_val_data = Data(sources[new_node_val_mask], destinations[new_node_val_mask],
    # #                             imestamps[new_node_val_mask],
    # #                             dge_idxs[new_node_val_mask], labels[new_node_val_mask])
    # # new_node_test_data = Data(sources[new_node_test_mask], destinations[new_node_test_mask],
    # #                               timestamps[new_node_test_mask], edge_idxs[new_node_test_mask],
    # #                                                                                                     t("The dataset has {} interactions, involving {} different nodes".format(full_data.n_interactions,
    # #                                                                                  full_data.n_unique_nodes))
    # #         t("The training dataset has {} interactions, involving {} different nodes".format(
    # #         train_data.n_interactions, train_data.n_unique_nodes))
    # #         t("The validation dataset has {} interactions, involving {} different nodes".format(
    # #         val_data.n_interactions, val_data.n_unique_nodes))
    # #         t("The test dataset has {} interactions, involving {} different nodes".format(
    # #         test_data.n_interactions, test_data.n_unique_nodes))
    # #         t("The new node validation dataset has {} interactions, involving {} different nodes".format(
    # #     new_node_val_data.n_interactions, new_node_val_data.n_unique_nodes))
    # #     print("The new node test dataset has {} interactions, involving {} different nodes".format(
    # #         _node_test_data.n_interactions, new_node_test_data.n_unique_nodes))
